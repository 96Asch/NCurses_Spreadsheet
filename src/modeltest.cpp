...
void testFormula() {
	...
	cell.set(new CellFormula(sheet, "=8*8"));
...
void testRange() {
	...
	Sheet sheet;
	sheet.getCell(0, 0).set(new CellValue<int>(1));
	sheet.getCell(1, 0).set(new CellValue<int>(2));
	sheet.getCell(2, 0).set(new CellValue<int>(3));
	sheet.getCell(3, 0).set(new CellValue<int>(4));
	Range range;

	Range::iterator it = range.rangeFromString("A1:A4", &sheet)->begin();
	...
	it = range.createRange(a1, a2, &sheet)->begin();

	sheet.getCell(1, 1).set(new CellValue<float>(9.6f));
	sheet.getCell(2, 1).set(new CellValue<int>(2));
	sheet.getCell(1, 2).set(new CellValue<std::string>("Hello"));
	sheet.getCell(2, 2).set(new CellValue<int>(99));
